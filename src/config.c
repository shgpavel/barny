...
barny_config_validate_font(const barny_config_t *config)
{
	...
	requested_family = pango_font_description_get_family(desc);
	if (!requested_family) {
		fprintf(stderr,
	...
	if (!font) {
		...
		actual_family = pango_font_description_get_family(actual);
...
barny_config_defaults(barny_config_t *config)
{
	...
	config->sysinfo_p_cores               = 0;
	config->sysinfo_e_cores               = 0;
...
config_replace_string_array(char ***items, int *count, const char *value)
{
	...
	if (items && *items) {
		prev = (count && *count > 0) ? (size_t)*count : 0;
	...
	parsed = barny_parse_csv(value, &new_count);
	if (items) {
		*items = parsed;
	} else {
...
parse_line(barny_config_t *config, const char *key, const char *value)
{
	if (strcmp(key, "height") == 0) {
		...
		config->modules_left = strdup(value);
	} else if (strcmp(key, "modules_center") == 0) {
		free(config->modules_center);
		config->modules_center = strdup(value);
                                 ...
                                 const char *modules_right)
{
	...
	fprintf(out, "\n# Module layout (generated by barny-layout-editor)\n");
