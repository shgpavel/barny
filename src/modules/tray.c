             ...
             int corner_radius, double r, double g, double b, double a)
{
	...
	if (square) {
		...
		if (corner_radius > 0) {
			...
			cairo_new_path(cr);
			cairo_arc(cr, left + cr_r, top + cr_r, cr_r, M_PI,
			          3 * M_PI / 2);
			cairo_arc(cr, left + w - cr_r, top + cr_r, cr_r,
			          3 * M_PI / 2, 0);
			cairo_arc(cr, left + w - cr_r, top + h - cr_r, cr_r, 0,
			          M_PI / 2);
			cairo_arc(cr, left + cr_r, top + h - cr_r, cr_r, M_PI / 2,
			          M_PI);
			cairo_close_path(cr);
		} else {
...
tray_render(barny_module_t *self, cairo_t *cr, int x, int y, int w, int h)
{
	...
	for (item = items; item; item = item->next) {
		...
		draw_icon_bg(cr, cx, cy, data->icon_size, square, corner_radius,
