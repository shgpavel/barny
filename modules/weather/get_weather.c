...
struct weather
get_weather(struct location ldata, char *API_KEY)
{
	int            _exit          = 0;
	char           template_str[] = "https://api.openweathermap.org/data"
	                                "/2.5/weather?lat=%lg&lon=%lg&appid=%s&units=metric";
	char           result_str[400];
	struct weather returned = { 0 };
	sprintf(result_str, template_str, ldata.latitude, ldata.longitude,
	        API_KEY);
